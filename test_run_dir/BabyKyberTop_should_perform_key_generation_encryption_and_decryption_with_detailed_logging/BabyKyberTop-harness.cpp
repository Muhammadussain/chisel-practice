...
#endif
#include <iostream>
class BabyKyberTop_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    BabyKyberTop_api_t(VBabyKyberTop* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_decryption_done)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_encryption_done)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_key_done)));
        ...
        sim_data.signal_map["BabyKyberTop.reset"] = 0;
    }
#if VM_TRACE
...
static BabyKyberTop_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VBabyKyberTop* top = new VBabyKyberTop;
    std::string vcdfile = "test_run_dir/BabyKyberTop_should_perform_key_generation_encryption_and_decryption_with_detailed_logging/BabyKyberTop.vcd";
    ...
    Verilated::mkdir("test_run_dir/BabyKyberTop_should_perform_key_generation_encryption_and_decryption_with_detailed_logging/logs");
    VerilatedCov::write("test_run_dir/BabyKyberTop_should_perform_key_generation_encryption_and_decryption_with_detailed_logging/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
