...
#endif
#include <iostream>
class kyber_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    kyber_api_t(Vkyber* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_dmemRes_bits_dataResponse)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_dmemRes_valid)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_dmemReq_ready)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_write_data)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_read_en)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_write_en)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_addr)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_dmemRes_ready)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_dmemReq_bits_isWrite)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_dmemReq_bits_activeByteLane)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_dmemReq_bits_dataRequest)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_dmemReq_bits_addrRequest)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_dmemReq_valid)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_read_data)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["kyber.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    Vkyber* top = new Vkyber;
    std::string vcdfile = "test_run_dir/KyberTop_Test/kyber.vcd";
    ...
    Verilated::mkdir("test_run_dir/KyberTop_Test/logs");
    VerilatedCov::write("test_run_dir/KyberTop_Test/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
